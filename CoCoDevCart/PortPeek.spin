...
PUB main
  ...
  dira := $00_FF   ' Lower 8 are output to PIA Port A
                   ' Upper 8 are inputes from PIA Port B
...
' when C64 starts up. This puts the cartridge in "bit" mode. For nibble transfers make sure
' outa is $00 when C64 starts up. This puts the cartridge in "nibble" mode.
' 
' You will have to program the PROP with the new outa value and power cycle the system.

' The C64 waits on the clock pin (Pin 10 PA7) to go high. This clock pin should be pulled to
' ground with a 1K resistor to prevent false-triggers when the propeller resets and floats
' its I/O pins.
